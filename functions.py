...
def printaPartida(partida):
    dictionary = {
        "valor_equipe_titular_man": "Valor equipe titular (MANDANTE)",
        "valor_equipe_titular_man": "Valor equipe titular (VISITANTE)",
        "rodada" : "Rodada",
        "estadio" : "Estádio",
        "arbitro" : "Árbitro",
        "publico" : "Público",
    ...
    print("--------------------------------------")
    print(f"BRASILEIRÃO {partida['ano_campeonato']}")
    print(f'{partida["time_man"]} {partida["gols_man"]} x {partida["gols_vis"]} {partida["time_vis"]}')
    print(f"Data do jogo: {partida['data'].strftime('%d/%m/%Y')} às {partida['horario']}")
    print("")
    for key in partida.keys():
        if key in dictionary:
            if (partida[key] != None and key != "id"):
                if (key == "valor_equipe_titular_man" or key == "valor_equipe_titular_vis"):
                    valor = partida[key];
                    valor = "{:,d}".format(valor).replace(",",".")
                    print(f"{dictionary[key]}: R$ {valor}")
                else:
                    print(f"{dictionary[key]}: {str(partida[key])}")
